...
function filterMeals() {
  ...
  const filteredMeals = allMeals.filter(
    (meal) => meal.strMeal.toLowerCase().includes(searchInput) // Іздеу мәтіні бар ма, жоқ па тексеру
  );
...
function updateCart() {
  ...
  cart.forEach((item, index) => {
    ...
    li.innerHTML = ` ${item.name} - ${item.price}₸ <button onclick="removeFromCart(${index})" style="background: red;">X</button>`;
